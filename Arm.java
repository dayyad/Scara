...
public class Arm
{
    //These are the callibrations in form of y=ab + c for each motor.
    double calibrateC1 = 1056.38;
    double calibrateC2 = 780;
    double calibrateA1 = 4.14;
    double calibrateA2 = 10;
    ...
    public void directKinematic(){
        ...
        if (d<2*r){
            ...
            double  h = Math.sqrt( Math.pow(r,2) - ( 0.25*( Math.pow((xj2-xj1),2) + Math.pow((yj2-yj1),2) ) ) );
            double alpha= Math.atan( (yj1-yj2) / (xj2-xj1) );
            // tool position
            double xt = xa + h*(Math.cos(Math.PI/2 - alpha));
            double yt = ya + h*(Math.sin(Math.PI/2 - alpha));
            double xt2 = xa - h*Math.cos(alpha-Math.PI/2); //math here correct? Shouldnt it be pi/2 - alpha?
            double yt2 = ya - h*Math.sin(alpha-Math.PI/2); //same here. Double check with tutor
        } else {
            valid_state = false;
        }
    ...
    // motor asngles from tool position
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double dx1 = xt - xm1; 
        double dy1 = yt - ym1;
        double alpha= Math.atan( (yt-ym2) / (xm2-xt) );
        ...
        // elbows positions
        xj1 = xm1 + r*Math.cos(theta1);
        yj1 = ym1 + r*Math.sin(theta2);

        theta1 = Math.atan2(yj1, xj1-xm1) ;
        ...
        // elbows positions
        xj2 = xm2 - r*Math.cos(180-Math.PI/theta2*180);
        yj2 = ym2 - r*Math.sin(180 - Math.PI/theta2*180);
        // motor angles for both 1st elbow positions
        theta2 = Math.atan2(yj2,xj2-xm2);
    ...
    public int get_pwm1(){
        int pwm = (int)(calibrateA1*theta1*(180/Math.PI) + calibrateC1);
        return pwm;
    }
    // ditto for motor 2
    public int get_pwm2(){
        int pwm =(int)(calibrateA2*theta2*(180/Math.PI) + calibrateC2);
        return pwm;
    }
